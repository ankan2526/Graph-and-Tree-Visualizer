...
let leftPoint = document.getElementById("leftpoint");
...
let nodes = [] , lines = [] , vis = [] , todrag , subx , suby , isTree , spaceleft , spacetop , leftpoint;
function Construction(e , N){
    ...
    Reset(nodes , lines);
    nodes = [] ; lines = [] ; todrag = null ; subx = 0 ; suby = 0 ; isTree = true;
    spaceleft = parseInt(deltax.value) , spacetop = parseInt(deltay.value) , leftpoint = parseInt(leftPoint.value);
    ...
    if(isTree == false){
        ...
        dfsConstruct(1 , -1 , leftpoint , 100);
        makeEdges();
        move();
    }
...
rootIt.addEventListener('click' , () => {
    ...
    let r = input.value;
    spaceleft = parseInt(deltax.value);
    spacetop = parseInt(deltay.value);
    leftpoint = parseInt(leftPoint.value);
    console.log(spaceleft , spacetop);
    r = parseInt(r);
    ...
    if(r > N || r < 1){
        ...
        if(isTree) {
            dfsConstruct(r , -1 , leftpoint , 100);
            makeEdges();
            move();
        }
        else{
